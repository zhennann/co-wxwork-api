...
class API {
  ...
  async request(opts, retry = 3) {
    ...
    return this.axiosInstance.request(opts).then(async res => {
      ...
      if (errcode === 40001 && retry > 0) {
        // 销毁已过期的token
        await this.saveToken(null, this.config.appName);
  ...
  async getAccessToken() {
    ...
    await this.saveToken({ accessToken: token.accessToken, expireTime: token.expireTime }, this.config.appName);
    return token;
  }
  ...
  async ensureAccessToken() {
    ...
    const token = await this.getToken(this.config.appName);
    let accessToken;
   ...
   * @return {Promise.<{ticket, expires_in}>}
   */
